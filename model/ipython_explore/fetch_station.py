...
def fetch_station(city, station_id, time_agg_level,agg_type=None):
    import os
    import psycopg2
    import pandas as pd
    import numpy as np
    ...
    if agg_type != None:
        ...
        new_data = station_df_non_zero_row.groupby(TimeGrouper(str(time_agg_level) + 'min'))
        def first_row(x):
            return x[0,:]
        station_bucketed = new_data.apply(first_row)
